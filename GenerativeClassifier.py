...
class GenerativeClassifier(nn.Module):
  ...
  def __init__(self, model, n_dim, n_classes, means, vars):
  ...
  def pdf(self, x):
    ...
    for i in range(self.n_classes):
      probs += pis[:,i]
      probs += N(self.means[i], self.vars[i]).log_prob(x).mean(axis=1)
  ...
  def forward(self, x):
    ...
    for i in range(self.n_classes):
      probs += pis[:,i]
      probs += MVN(self.means[i], torch.diag(self.vars[i])).log_prob(x)
