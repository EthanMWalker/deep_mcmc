...
def acceptance(x, y):
  ...
  p = torch.tanh(a / b)
  # p = a - b
  return bernoulli.rvs(p.item())


def nextState(x):
  K = 784
  y = np.random.multivariate_normal(x.detach().cpu().numpy().reshape(784), np.eye(K))
...
samples, logprobs = metropolis(torch.rand(1,1,28,28).to(device), 100)
# samples, logprobs = metropolis(mean[0].view(1,1,28,28).to(device), 10000)
